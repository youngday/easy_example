...
async fn get_case_count() -> Result<u32> {
    let (mut socket, _) = connect_async("ws://localhost:9002/getCaseCount").await?;
...
async fn update_reports() -> Result<()> {
    let (mut socket, _) =
        connect_async(&format!("ws://localhost:9002/updateReports?agent={}", AGENT)).await?;
    socket.close(None).await?;
    Ok(())
}
...
async fn run_test(case: u32) -> Result<()> {
    ...
    let case_url = &format!("ws://localhost:9002/runCase?case={}&agent={}", case, AGENT);
